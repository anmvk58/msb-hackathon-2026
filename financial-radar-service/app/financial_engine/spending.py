...
def calculate_budget_guardrail(
    context: FinancialContext, customer_id: str, category: Category
) -> BudgetGuardrailResult:
...
def detect_spending_anomaly(
    ...
    *,
    ...
    if context.customer.customer_id != customer_id:
        raise ValueError(f"Unknown customer_id: {customer_id}")
    transactions = [
        item
        ...
        and item.category not in {Category.RENT, Category.TRANSFER}
    ]
