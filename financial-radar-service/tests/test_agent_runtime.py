...
def test_action_not_executed_before_confirmation(session: Session, fake_banking_gateway: FakeBankingGateway) -> None:
...
def test_action_log_created_after_execution(session: Session, fake_banking_gateway: FakeBankingGateway) -> None:
    runtime = LocalAgentRuntime()
    recommendation = runtime.run(
        session,
        customer_id="C001",
    ...
    assert waiting.action_id in fake_banking_gateway.idempotent_results
    assert log.latency_ms is not None


def test_declined_action_is_never_executed(session: Session, fake_banking_gateway: FakeBankingGateway) -> None:
    runtime = LocalAgentRuntime()
    recommendation = runtime.run(
        session,
        customer_id="C001",
