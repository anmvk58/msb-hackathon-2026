...
from app.llm import GreenNodeLLMClient, LLMStructuredOutputError, MockLLMClient
...
def test_runtime_factory_local() -> None:
    ...
    assert isinstance(runtime.llm, MockLLMClient)


def test_runtime_factory_local_with_real_greennode_llm() -> None:
    settings = greennode_settings(agent_runtime="local")
    runtime = build_agent_runtime(
        settings=settings, llm_transport=decision_transport()
    )
    assert isinstance(runtime, LocalAgentRuntime)
    assert not isinstance(runtime, GreenNodeAgentRuntime)
    assert isinstance(runtime.llm, GreenNodeLLMClient)
...
def test_local_greennode_provider_also_requires_llm_credentials() -> None:
    with pytest.raises(ValueError, match="missing.*LLM_API_KEY"):
        Settings(
            _env_file=None,
            agent_runtime="local",
...
def test_llm_cannot_introduce_unknown_option(session: Session, fake_banking_gateway: FakeBankingGateway) -> None:
    plan = CandidateActionGenerator(fake_banking_gateway).generate(
        session,
        customer_id="C001",
        as_of=date(2026, 9, 1),
...
def test_confirmation_still_required_with_greennode_runtime(
    session: Session, fake_banking_gateway: FakeBankingGateway,
) -> None:
    runtime = build_agent_runtime(
        settings=greennode_settings(), llm_transport=decision_transport()
    )
    ...
    assert fake_banking_gateway.get_financial_context("C001").active_budgets == []
    ...
    budget = fake_banking_gateway.get_financial_context("C001").active_budgets[0]
