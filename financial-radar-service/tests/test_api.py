...
def test_home_guides_users_to_documentation(session: Session) -> None:
    with client_for(session) as client:
        response = client.get("/")
    app.dependency_overrides.clear()
    assert response.status_code == 200
...
def test_openapi_documents_workflow_examples_and_demo_scope() -> None:
    ...
    assert "Cách thử nhanh" in schema["info"]["description"]
    assert "không phải Core Banking MSB thật" in schema["info"]["description"]
    ...
    assert "Mock Core Banking" in budget_operation["description"]
